...
date: 2014-01-18T00:24:00.000Z
slug: api-authentication-with-liberator-and-friend
tags:
  - code
  - clojure
categories:
...
From the [project site](http://clojure-liberator.github.io/liberator/):
...
This middleware sits over top of all of your resources and routing layer (typically handled with a library like [Compojure](https://github.com/weavejester/compojure)), and provides all of the plumbing necessary for authorization and authentication.
...
(That's a [namespace-qualified keyword](https://kotka.de/blog/2010/05/Did_you_know_III.html), by the way.)
...
([Here's the code](https://github.com/cemerick/friend/blob/master/src/cemerick/friend/workflows.clj#L35) if you want to follow along.)
...
Resources typically handle authorization with some function of the `::friend/identity` that the middleware added to the session. If the identity is missing (IE, the request isn't authenticated) or the identity doesn't have the required permissions, the resource can throw an exception with [cemerick.friend/throw-unauthorized](https://github.com/cemerick/friend/blob/master/src/cemerick/friend.clj#L256) with the identity that didn't pass the check. (This might be nil, of course.) This function throws an exception with some special metadata.
...
So I wrote [a pull request](https://github.com/clojure-liberator/liberator) that extends Liberator's resources to accept a `:base` key. The `:base` key takes a map of liberator decision points and creates your resource by merging the other kv pairs into these defaults.
