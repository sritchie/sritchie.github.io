...
date: 2021-01-11T22:20:41.000Z
slug: dual-numbers-and-automatic-differentiation
tags:
  - code
...
categories:
  - math-and-physics
description: "This literate essay develops an implementation of a type called `Differential`. A `Differential` is a generalization of a type called a [\"dual number\"](https://en.wikipedia.org/wiki/Dual_number), and the glowing, pulsing core of the SICMUtils implementation of forward-mode automatic differentiation."
math: true
---
...
The technique of evaluating a function and its derivative in parallel is called &quot;forward-mode [Automatic Differentiation](https://en.wikipedia.org/wiki/Automatic_differentiation)&quot;. The [SICMUtils wiki](https://github.com/sicmutils/sicmutils/wiki/Automatic-Differentiation) has more information on the history of this technique, and links to the many other implementations you'll find in different languages. See the [cljdocs Automatic Differentiation page](https://cljdoc.org/d/sicmutils/sicmutils/CURRENT/doc/calculus/automatic-differentiation) for &quot;how do I use this?&quot;-style questions.
...
The problem here is called &quot;perturbation confusion&quot;, and is covered beautifully in [&quot;Confusion of Tagged Perturbations in Forward Automatic Differentiation of Higher-Order Functions&quot;](https://arxiv.org/abs/1211.4892), by Manzyuk et pal. (2019).

The solution is to introduce a new $\varepsilon$ for every level, and allow different $\varepsilon$ instances to multiply without annihalating. Each $\varepsilon$ is called a &quot;tag&quot;. [Differential](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L568) (implemented below) is a generalized dual number that can track many tags at once, allowing nested derivatives like the one described above to work.
...
But this implies that [extract-tangent](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L361) from the example above needs to know how to handle vectors and other collections; in the case of a vector `v` by returning `(mapv extract-tangent v)`

The dual number implementation is called [Differential](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L568); the way that [Differential](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L568) instances interact with container types in SICMUtils makes it easy for these captures to occur all over. Whenever we multiply a [Differential](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L568) by a structure, a function, a vector, any of those things, our implementation of the SICMUtils generics pushes the [Differential](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L568) inside those objects, rather than forming a [Differential](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L568) with, for example, a vector in the primal and tangent parts.
...
NOTE: The real implementation is more subtle! See the `sicmutils.calculus.derivative` namespace for the actual implementation of [IPerturbed](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L350) for functions and multimethods.

All of this suggests that we need to make [extract-tangent](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L361) an open function that other folks can extend for other container-like types ([functors](https://en.wikipedia.org/wiki/Functor), specifically).

The [IPerturbed](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L350) protocol accomplishes this, along with two other functions that we'll use later:
...
`replace-tag` exists to handle subtle bugs that can arise in the case of functional return values. See the &quot;Amazing Bug&quot; sections in [sicmutils.calculus.derivative-test](https://github.com/sicmutils/sicmutils/blob/940b83babd0c6f6d556987ed7c835936671c2a89/test/sicmutils/calculus/derivative_test.cljc) for detailed examples on how this might bite you.
...
The set of tags is implemented as a &quot;vector set&quot;, from [sicmutils.util.vector-set](https://github.com/sicmutils/sicmutils/blob/940b83babd0c6f6d556987ed7c835936671c2a89/src/sicmutils/util/vector_set.cljc). This is a sorted set data structure, backed by a Clojure vector. vector sets provide cheap &quot;max&quot; and &quot;min&quot; operations, and acceptable union, intersection and difference performance.
...
Finally, the function we've been waiting for! `bundle` allows you to augment some non-[Differential](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L568) thing with a tag and push it through the generic arithmetic system, where it will accumulate the derivative of your original input (tagged with `tag`.)
...
A reminder: the `primal-part` of a [Differential](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L568) is all terms except for terms containing `max-order-tag`, and `tangent-part` is a [Differential](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L568) built out of the remaining terms, all of which contain that tag.
...
- `eq` and `compare-full` similarly looks at every component in the [Differential](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L568) supplied to both sides
...
There is yet another subtlety here, noted in the docstrings below. [lift-1](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L1135) and [lift-2](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L1158) really are able to lift functions like `clojure.core/+` that can't accept [Differential](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L568)s. But the first-order derivatives that you have to supply do have to be able to take [Differential](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L568) instances.

This is because the `tangent-part` of [Differential](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L568) might still be a [Differential](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L568), and for `Df` to handle this we need to be able to take the second-order derivative.
...
This first case of `g/expt`, where the exponent itself is non-[Differential](https://github.com/sicmutils/sicmutils/blob/c89b45d453b08a1e1259ff5de9d9841874807fda/src/sicmutils/differential.cljc#L568), is special-cased and slightly simpler. The second partial derivative throws, since the more general definition below should always override.
